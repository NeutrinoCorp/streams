...
func (l listenerDriverNoop) ExecuteTask(_ context.Context, _ ListenerTask) error {
	go func() {}()
	return nil
}
...
func (l listenerDriverNoopLoop) ExecuteTask(ctx context.Context, _ ListenerTask) error {
	go func() {
		for {
			select {
			case <-ctx.Done():
