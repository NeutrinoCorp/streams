...
func (r *StreamRegistry) Set(message interface{}, metadata StreamMetadata) {
	...
	r.SetByString(strings.TrimPrefix(msgType.String(), "*"), metadata)
}
...
func (r *StreamRegistry) Get(message interface{}) (StreamMetadata, error) {
	msgType := strings.TrimPrefix(reflect2.TypeOf(message).String(), "*")
...
func (r *StreamRegistry) GetByString(key string) (StreamMetadata, error) {
	metadata, ok := r.registry.Get(key)
	if !ok {
		return StreamMetadata{}, ErrMissingStream
	}
	return metadata.(StreamMetadata), nil
}
...
//
// If metadata is not found by the given key, then fallback default to O(log n) lookup.
// This will increase time complexity of the fallback function by the GetByString base complexity.
...
//
// This optimistic lookup is done in order to keep amortized time complexity when using non-reflection
...
func (r *StreamRegistry) GetByStreamName(name string) (StreamMetadata, error) {
	...
	var res StreamMetadata
	_, val := r.registry.Find(func(_ interface{}, value interface{}) bool {
		val := value.(StreamMetadata)
		return val.Stream == name
	})
	if val == nil {
		return StreamMetadata{}, ErrMissingStream
	}
	res = val.(StreamMetadata)
	return res, nil
}
